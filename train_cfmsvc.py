...
def train(rank, world_size, experiment_path, logger, args, h):
    ...
    for epoch in range(num_epoches):
        ...
        for i, batch in enumerate(train_loader):
            ...
            x0 = torch.rand_like(wav_gt)
            x1 = wav_gt
            mu_t = t.unsqueeze(1) * x1 + (1 - t.unsqueeze(1)) * x0
            sigma_t = sigma
            x = mu_t + sigma_t * torch.randn(N, wav_gt.shape[-1]).to(device)
            ut = x1 - x0

            vt, mel_rec = svc(lf0.unsqueeze(1).to(device),ppg.to(device),pho.to(device),t, x)

            loss = F.mse_loss(vt.squeeze(1) * wav_mask, 
                                     ut * wav_mask) * 10
    ...
    save_checkpoint(os.path.join(save_path,f"f_{epoch:03d}.pt"),
            {'svc': (svc.module if world_size > 1 else svc).state_dict()})
