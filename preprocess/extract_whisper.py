...
def whisper_encoder(audio_paths):
    ...
    ## expand consecutive whisper features since whisper model builds on 30s audio
    features_dup = torch.zeros((batch, 3000, 1024), dtype=torch.float, device=model.device)
    features_dup[:,0::2,:] = features
    features_dup[:,1::2,:] = features
    features, _ = pad_or_trim(features_dup,length=WHISPER_SEQ,axis=1) 
    return features.cpu().detach().numpy()



def extract_whisper_features(dataset, dataset_type, batch_size=25):
    print("-" * 20)
